...
function ChallanActions({
  ...
  return (
    ...
    >
      ...
      <Link href={`/returnable-challans/${id}/edit`}>
...
export default function ReturnableChallansPage() {
  ...
  const [pdfViewerFilename, setPdfViewerFilename] = useState('returnable-challan.pdf')
  ...
  const handleView = (c: Challan) => {
    ...
    setPdfViewerUrl(`/api/returnable-challans/${c.id}/pdf`)
    setPdfViewerOpen(true)
  }

  const handleDelete = async (id: string, challanNo: string) => {
    if (!confirm(`Delete returnable challan "${challanNo}"?`)) return
    const res = await fetch(`/api/returnable-challans/${id}`, { method: 'DELETE' })
    if (res.ok) {
      toast({ title: 'Returnable challan deleted' })
      fetchChallans()
    } else {
  ...
  return (
    <div className="space-y-4 md:space-y-6 min-w-0">
      <ListPageToolbar
        searchPlaceholder="Search returnable challans..."
        search={search}
        onSearchChange={(v) => { setSearch(v); setPage(1) }}
        addLabel="New RC"
        addHref="/returnable-challans/new"
        viewMode={viewMode}
      ...
      {showCards && (
        <div className="space-y-3">
          {loading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">No returnable challans found</CardContent>
            </Card>
          ) : (
