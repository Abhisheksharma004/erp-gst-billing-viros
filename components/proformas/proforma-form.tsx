...
import { proformaSchema, type ProformaInput } from '@/lib/validations'
...
export function ProformaForm({ mode, proformaId, fromQuotationId }: ProformaFormProps) {
  ...
  const { register, control, handleSubmit, watch, setValue, reset, formState: { errors } } = useForm<ProformaInput>({
  ...
  useDefaultDocumentTerms('proforma', !isEdit, applyDefaultTerms)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true)
      try {
        const [pRes, prodRes, cRes] = await Promise.all([
          fetch(`/api/proformas/${proformaId}`),
          fetch('/api/products?limit=500'),
          fetch('/api/customers?limit=200'),
        ])
        if (!pRes.ok) throw new Error('Proforma not found')
        ...
        setProducts(productList)
        setCustomers(customerList)
        setProformaNo(p.proforma_no ?? null)

        const partyDetails = parseQuotationPartyDetails(p.party_details)
        const buyerFromDb = partyDetails?.buyer
        const consigneeFromDb = partyDetails?.consignee

        const buyer: PartyFields = {
          name: buyerFromDb?.name || p.customer_name || '',
          contactPerson: buyerFromDb?.contactPerson || p.customer_contact_person || '',
          address: buyerFromDb?.address || p.customer_address || '',
          mobile: buyerFromDb?.mobile || p.customer_mobile || p.customer_phone || '',
          gstin: buyerFromDb?.gstin || p.customer_gstin || '',
          pan: buyerFromDb?.pan || p.customer_pan || '',
          city: buyerFromDb?.city || p.customer_city || '',
        }
        ...
        const consignee: PartyFields = {
          ...
          address:
            consigneeFromDb?.address ||
            p.customer_shipping_address ||
            buyer.address,
          mobile: consigneeFromDb?.mobile || buyer.mobile,
          gstin: consigneeFromDb?.gstin || buyer.gstin,
          pan: consigneeFromDb?.pan || buyer.pan,
          city: consigneeFromDb?.city || p.customer_shipping_city || buyer.city,
        }
        ...
        reset({
          ...
          date: toDateInputValue(p.date) || today,
          validUntil: p.valid_until ? toDateInputValue(p.valid_until) : undefined,
        ...
        if (cancelled) return
        const message = e instanceof Error ? e.message : 'Failed to load proforma'
        toast({ title: message, variant: 'destructive' })
  ...
  }, [isEdit, proformaId, reset, today, toast, router])
  ...
  const validateBeforeSubmit = (data: ProformaInput): string | null => {
    if (!data.date || !/^\d{4}-\d{2}-\d{2}$/.test(data.date)) {
      return 'Please select a valid proforma date'
    }
    if (!data.validUntil || !/^\d{4}-\d{2}-\d{2}$/.test(data.validUntil)) {
      return 'Please select a Valid Till date'
    }
    if (data.validUntil < data.date) {
      return 'Valid till must be on or after the proforma date'
    }
  ...
  const onSubmit = async (data: ProformaInput) => {
    ...
    try {
      ...
      const url = isEdit ? `/api/proformas/${proformaId}` : '/api/proformas'
      ...
      toastSuccessNavigate(
        isEdit ? 'Proforma updated' : 'Proforma created successfully',
        () => router.push('/proformas')
      )
    } catch (e: unknown) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-24 text-muted-foreground">
        ...
        <p className="text-sm">Loading proforma...</p>
      </div>
    )
  }
    ...
    <div className="space-y-6 max-w-6xl min-w-0">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={gstType}
                  onValueChange={(v) => setValue('gstType', v as ProformaInput['gstType'])}
                >
        ...
        <div className="grid grid-cols-2 gap-3 pt-2 border-t sm:flex sm:justify-end">
          <Link href="/proformas" className="min-w-0">
