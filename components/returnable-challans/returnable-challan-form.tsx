...
export interface ReturnableChallanFormProps {
  mode: 'create' | 'edit'
  challanId?: string
}
...
export function ReturnableChallanForm({ mode, challanId }: ReturnableChallanFormProps) {
  ...
  useDefaultDocumentTerms('returnable-challan', !isEdit, applyDefaultTerms)
    ...
    const load = async () => {
      ...
      try {
        const [cRes, pRes, custRes] = await Promise.all([
          fetch(`/api/returnable-challans/${challanId}`),
          fetch('/api/products?limit=500'),
          fetch('/api/customers?limit=200'),
        ])
        if (!cRes.ok) throw new Error('Returnable challan not found')
        ...
        const formItems = rawItems.map((item: {
          ...
          quantity: Number(item.quantity_issued) || 1,
        ...
        reset({
          ...
          completionDate: data.return_date ? toDateInput(data.return_date) : undefined,
          terms: data.terms || '',
        ...
        if (cancelled) return
        const message = e instanceof Error ? e.message : 'Failed to load returnable challan'
  ...
  const onSubmit = async (data: ChallanInput) => {
    ...
    try {
      ...
      const url = isEdit ? `/api/returnable-challans/${challanId}` : '/api/returnable-challans'
      ...
      if (!res.ok) {
        ...
        throw new Error(
          typeof e.error === 'string'
            ? e.error
            : isEdit
              ? 'Failed to update returnable challan'
              : 'Failed to create returnable challan'
        )
      }
      toastSuccessNavigate(
        isEdit ? 'Returnable challan updated' : 'Returnable challan created',
        () => router.push('/returnable-challans')
      )
    } catch (e: unknown) {
  ...
  if (loadingInitial) {
    return <FormPageLoader title="returnable challan form" />
  }

  return (
    <div className="space-y-6 max-w-5xl w-full min-w-0">
      <div className="flex items-center gap-3 sm:gap-4">
        <Link href="/returnable-challans">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">
            {isEdit ? `Edit Returnable Challan${challanNo ? ` — ${challanNo}` : ''}` : 'New Returnable Challan'}
          </h1>
          <p className="text-sm text-muted-foreground">
            {isEdit ? 'Update returnable challan details' : 'Create a returnable challan for goods issued on return basis'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
        <Card className="shadow-sm">
          ...
          <CardContent className="px-4 sm:px-6">
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="space-y-1.5 sm:space-y-2 min-w-0">
                <Label className="text-xs sm:text-sm">Expected Return Date</Label>
                <Input
                  type="date"
                  className="h-9 w-full min-w-0 text-sm px-2 sm:px-3"
        ...
        <div className="grid grid-cols-2 gap-3 pb-2 sm:pb-0">
          ...
          <Button type="submit" disabled={saving} className="w-full">
            {saving ? (isEdit ? 'Saving...' : 'Creating...') : isEdit ? 'Save Changes' : 'Create RC'}
          </Button>
        </div>
      </form>
    </div>
