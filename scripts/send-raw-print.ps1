...
public class RawPrinterHelper {
    ...
    public static bool SendBytesToPrinter(string szPrinterName, byte[] pBytes, string docName) {
        ...
        bool bSuccess = false;
        di.pDocName = string.IsNullOrEmpty(docName) ? "Direct QR Label" : docName;
        di.pDataType = "RAW";
    ...
    public static bool SendStringToPrinter(string szPrinterName, string szString, string docName) {
        ...
        return SendBytesToPrinter(szPrinterName, bytes, docName);
    }
}
"@
...
if ($FilePath -and (Test-Path $FilePath)) {
    ...
    $res = [RawPrinterHelper]::SendBytesToPrinter($PrinterName, $bytes, "Label Print Job")
    Write-Output $res
} elseif ($RawContent) {
    $res = [RawPrinterHelper]::SendStringToPrinter($PrinterName, $RawContent, "Label Print Job")
    Write-Output $res
} else {
